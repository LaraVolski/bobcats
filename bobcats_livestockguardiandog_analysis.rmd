...
#Phase1
detection.maps <- detectionMaps(CTtable = cam.operation.all.phases, 
...
for (species in c("Bat", "Bird", "Jack_Rabbit", "Pig", "Raccoon", "Rodent", "Skunk", "Squirrel", "Turkey", "Unknown")) {
  record.table.all <- record.table.all[record.table.all$Species != species, ]
}
...
test <- rai.calculate(record.table = record.table.all, start.date = "2016-03-22", end.date = "2018-01-17", camop = camera.operation.matrix, timeperiod.name = "test")
```
